...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            y_pred=self.hidden_layer.forward(x_data)
            y_pred = self.output_layer.forward(y_pred)
            return y_pred
    ...
    model.hidden_layer.W[:, 0] = [Wh11,Wh12]        # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [Wh21,Wh22]       # wagi neuronu h2
    model.hidden_layer.b[:] = [bh1,bh2]         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [Wo11,Wo21]       # wagi neuronu wyjściowego
    model.output_layer.b[:] = bo         # bias neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 201096         # Twój numer indeksu, np. 102247
