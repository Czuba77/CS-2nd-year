...
% Oznaczonie TODO wskazuje linijki kodu wymagające zmian
    ...
    cond = false;
    while cond==false
        node_counts(1)=randi(14)+2;
        node_counts(2)=randi(14)+2;
        while node_counts(1) == node_counts(2)
            node_counts(2)=randi(14)+2;
        end
        node_counts(3)=randi(14)+2;
        while node_counts(1) == node_counts(3) || node_counts(2) == node_counts(3)
            node_counts(3)=randi(14)+2;
        end
        node_counts(4)=randi(14)+2;
        while node_counts(1) == node_counts(4) || node_counts(2) == node_counts(4)  || node_counts(3) == node_counts(4)
            node_counts(4)=randi(14)+2;
        end
    ...
    for i = 1:length(node_counts)
        ...
        coefficients_runge = V{i}\y_runge; % V{i}\y_runge; % TODO
        ...
        plot(x_fine, interpolated_runge{i}, 'DisplayName', ['N = ' num2str(N)]);
        hold on;

        subplot(2,1,2);
        % TODO: plot interpolated_sine{i}
        plot(x_fine, interpolated_sine{i}, 'DisplayName', ['N = ' num2str(N)]);
        hold on;
    end
...
function V = get_vandermonde_matrix(x)
    % Buduje macierz Vandermonde’a w postaci rosnących potęg (0,1,2,...)
    N = length(x);
    V = zeros(N, N);
    for i = 1:N
        for j = 1:N
