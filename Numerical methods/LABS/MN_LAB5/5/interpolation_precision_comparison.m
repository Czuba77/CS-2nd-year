...
% y_mix - wartości double wielomianu określonego przez double(coef_vpa)
    ...
    y_nodes_inv = y_nodes';
    coef_double = V_double \ y_nodes_inv; % TODO
    ...
    y_nodes_vpa_inv = y_nodes_vpa';
    coef_vpa = V_vpa \ y_nodes_vpa_inv; % TODO
    ...
    legend show
    title('Interpolacja za pomocą zmiennych double')
    xlabel('x')
    ylabel('y')
    ...
    axis([-1 1 -2 2])
    hold on
    plot(x_fine, double(y_vpa), 'b', 'LineWidth', 1.5, 'DisplayName', 'Interpolacja vpa')
    legend show
    title('Interpolacja za pomocą zmiennych vpa')
    xlabel('x')
    ylabel('y')
    ...
    axis([-1 1 -2 2])
    hold on
    plot(x_fine, y_mix, 'g', 'LineWidth', 1.5, 'DisplayName', 'Interpolacja mix (vpa → double)')
    legend show
    title('Interpolacja z konwersją vpa na double')
    xlabel('x')
    ylabel('y')

    set(gcf, 'Position', [500 250 1000 750]);
