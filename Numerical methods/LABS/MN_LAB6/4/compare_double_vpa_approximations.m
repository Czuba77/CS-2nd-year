...
%       z zastosowaniem zmiennych vpa.
    ...
    fine_dates = linspace(dates(1), dates(end), length(ya));
    ya_clipped = max(min(ya, ymax), ymin);
    figure;
    subplot(3, 1, 1);
    plot(dates, y, '-', 'DisplayName', 'Dane oryginalne'); hold on;
    plot(fine_dates, ya_clipped, '-', 'LineWidth', 1.5, 'DisplayName', 'Aproksymacja (double)');
    title('Aproksymacja w podwójnej precyzji (double)');
    xlabel('Data');
    ...
    fine_dates = linspace(dates(1), dates(end), length(yv));
    yv_clipped = max(min(yv, ymax), ymin);  
    plot(fine_dates, yv_clipped, 'r-', 'LineWidth', 1.5, 'DisplayName', 'Aproksymacja (vpa)');
    title('Aproksymacja z zastosowaniem vpa');
    xlabel('Data');
    ylabel('Produkcja energii');
    legend('Location', 'best');
    grid on;
