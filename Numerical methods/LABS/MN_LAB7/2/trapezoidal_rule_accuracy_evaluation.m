function [ft_5, integral_1000, Nt, integration_error] = trapezoidal_rule_accuracy_evaluation()
    ...
    for i = 1:length(Nt)
        ...
        integration_result=trapezoidal_rule(x);
    ...
    title('Błąd całkowania metodą trapezów w funkcji liczby podprzedziałów');
    grid on;
...
function integral_approximation = trapezoidal_rule(x)
    ...
    n = length(x);
    integral_approximation =failure_density_function(x(1))/2 + failure_density_function(x(length(x)))/2;
    deltaX = x(2) - x(1);
    for i = 2:n-1
        integral_approximation= integral_approximation+ failure_density_function(x(i));
    end
    integral_approximation=integral_approximation*deltaX;
end
