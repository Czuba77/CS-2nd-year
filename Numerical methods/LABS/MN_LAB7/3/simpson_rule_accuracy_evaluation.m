function [ft_5, integral_1000, Nt, integration_error] = simpson_rule_accuracy_evaluation()
    ...
    for i = 1:length(Nt)
        ...
        integration_result=simpson_rule(x);
    ...
    title('Błąd całkowania metodą Simpsona w funkcji liczby podprzedziałów');
    grid on;
...
function integral_approximation = simpson_rule(x)
    ...
    deltaX = x(2) - x(1);
    for i = 1:n-1
        integral_approximation= integral_approximation + failure_density_function(x(i)) + 4*failure_density_function((x(i)+x(i+1))/2) + failure_density_function(x(i+1));
    end
    integral_approximation=integral_approximation*deltaX/6;
end
