function [ft_5, yrmax, Nt, xr, yr, integration_error] = monte_carlo_accuracy_evaluation()
    ...
    x_dense = linspace(0, 5, 10000);
    y_values = failure_density_function(x_dense);
    M = max(y_values); 
    ...
    integration_error = [];
    for i = 1:length(Nt)
        [integration_result,x, y]=monte_carlo_integral(Nt(i),xrmax,yrmax);
        xr{i} = x;
        yr{i} = y;
        integration_error(1, i) = abs(integration_result - reference_value);
    end
    ...
    title('Błąd całkowania metodą Monte Carlo w funkcji liczby losowań');
    grid on;
...
function [integral_approximation, x, y] = monte_carlo_integral(N, xmax, ymax)
    ...
    N1=0;
    for i = 1:N
        if failure_density_function(x(i))>= y(i)
            N1=N1+1;
        end
    end
